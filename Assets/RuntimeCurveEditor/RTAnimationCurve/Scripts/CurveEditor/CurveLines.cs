...
// Copyright © 2013-2019 Rus Artur PFA
...
namespace RuntimeCurveEditor
{
    ...
    public class CurveLines : MonoBehaviour, InterfaceContextMenuListener, InterfaceKeyEditListener, InterfacePostRenderer
    {
        ...
        void Start() {
            ...
            if(GetComponent<PostRenderBehaviour>() != null)
                GetComponent<PostRenderBehaviour>().enabled = true;
            if (GetComponent<JPostRenderBehaviour>() != null)
                GetComponent<JPostRenderBehaviour>().enabled = true;
        }
        ...
        void Update() {
            ...
            if (ActiveCurveForm.curve1 != null) {
                if (ActiveCurveForm.firstCurveSelected ? (ActiveCurveForm.curve1.length != ActiveCurveForm.curve1KeysCount) :
                    ...
                    selectedKeyIndex = -1;//just be sure that selected key is not an out of range key
                    ...
                    if (!isTangentSelected) {
                        ...
                        if ((ActiveCurveForm.curve1 != null) && (gridRect.xMin - marginPixels < mousePos.x) && (gridRect.xMax + marginPixels > mousePos.x) &&
                            ...
                            if (multipleKeySelection.MultipleKeysAreSelected()) {
                                if (Input.GetMouseButtonDown(0) && multipleKeySelection.InsideSelectedKeys()) {
                            ...
                            if (!multipleKeysMove && !anyCurveSelected && Input.GetMouseButtonDown(0)) {
        ...
        void DrawHorizontalLines() {
            ...
            for (int i = 0; i <= mHorLines; i++) {
                ...
                GL.Begin(GL.LINES);
                DrawLine(i, mStartHor, mSampleHor, EntireRect.yMax);
                if ((i != 0) && mMirroredHor) {
                    DrawLine(i, mStartHor, -mSampleHor, EntireRect.yMin);
                }
                GL.End();
                ...
                if (LastLineRezid(i)) {
                    ...
                    lineMaterial.color = lineColor;
                    lineMaterial.SetPass(0);
                    DrawRezidualLine(EntireRect.yMax);
                    if (mMirroredHor) {
                        DrawRezidualLine(EntireRect.yMin);
                    }
                    GL.End();
                }
            }
        ...
        void DrawLine(int i, float start, float sample, float limit) {
            float gradation = LastLine(i) ? limit : (start + i * sample);
            if ((adjGridRect.yMin <= gradation) && (gradation <= adjGridRect.yMax)) {
                GL_Vertex3(gridRect.xMin, gradation, 0);
                GL_Vertex3(gridRect.xMax, gradation, 0);
            }
        }

        void DrawRezidualLine(float limit) {
            if ((adjGridRect.yMin <= limit) && (limit <= adjGridRect.yMax)) {
                GL_Vertex3(gridRect.xMin, limit, 0);
                GL_Vertex3(gridRect.xMax, limit, 0);
            }
        }
        ...
        void DrawVerticalLines() {
            ...
            for (int i = 0; i <= vertLines; i++) {
                ...
                if ((adjGridRect.xMin <= gradation) && (gradation <= adjGridRect.xMax)) {
                    ...
                    GL_Vertex3(gradation, gridRect.yMin, 0);
                    GL_Vertex3(gradation, gridRect.yMax, 0);
                    GL.End();
                }
            }
        }
        ...
        void DrawBasicShapes() {
            if (gradRect.height > 0f) {
                ...
                for (int i = 0; i < SHAPE_COUNT; ++i) {
                    ...
                    //tODO shapeRect should be calculated only once, on grid update
                    ...
                    lineMaterial.SetPass(0);
                    GL_Vertex3(shapeRect.xMin, shapeRect.yMin, 0);
                    GL_Vertex3(shapeRect.xMin, shapeRect.yMax, 0);
                    GL_Vertex3(shapeRect.xMax, shapeRect.yMax, 0);
                    GL_Vertex3(shapeRect.xMax, shapeRect.yMin, 0);
                    GL.End();
                    ...
                    Curves.DrawCurveForm(light, basicShapes[i], null, false, false, -1, shapeRect, shapeRect, normalRect, true, clip);
                }
            }
        }
        ...
        void DrawCurves() {
            ...
            if (activeCurve != null) {
                foreach (CurveForm curveForm in curveFormList) {
                    ...
                    Curves.DrawCurveForm(curveForm.shadyColor, curveForm.curve1, curveForm.curve2, false, false, selectedKeyIndex, EntireRect, adjGridRect, curveForm.gradRect, false, 1, drawPoints);
                }
                Curves.DrawCurveForm(ActiveCurveForm.color, ActiveCurveForm.curve1, ActiveCurveForm.curve2, ActiveCurveForm.firstCurveSelected, !ActiveCurveForm.firstCurveSelected, selectedKeyIndex, EntireRect, adjGridRect, gradRect, false, 1, drawPoints);
            }
        }
