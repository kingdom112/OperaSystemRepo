...
public static class IOHelper
{
    ...
    public static void SetData(string fileName, object pObject, string pkey = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
    {
        //将对象序列化为字符串
        string toSave = SerializeObject(pObject);
        //对字符串进行加密,32位加密密钥
        toSave = RijndaelEncrypt(toSave, pkey);
        StreamWriter streamWriter = File.CreateText(fileName);
        streamWriter.Write(toSave);
        streamWriter.Close();
    }

    public static object GetData(string fileName, Type pType, string pkey = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
    {
        StreamReader streamReader = File.OpenText(fileName);
        string data = streamReader.ReadToEnd();
        //对数据进行解密，32位解密密钥
        data = RijndaelDecrypt(data, pkey);
        streamReader.Close();
