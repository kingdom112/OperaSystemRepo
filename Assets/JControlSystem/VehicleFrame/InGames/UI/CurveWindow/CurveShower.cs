...
public class CurveShower : MaskableGraphic
{
    ...
    public float y_division
    {
        get
        {
            return Mathf.Abs(y_max - y_min);//返回差值的绝对值
        }
    }
    ...
    {
        return m_Curve.Evaluate((time_max - time_min) * x + time_min + time_offset);
    }
    private float CalcY(float x, float ymin)
    {
        return ((m_Curve.Evaluate((time_max - time_min) * x + time_min + time_offset) - ymin) / y_division - rectTransform.pivot.y);
    }
