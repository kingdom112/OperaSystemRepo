...
public class actionEvaluation : MonoBehaviour
{
    ...
    public void calculateDtwDiff(List<KeyValuePair<float, float>> timePairs)//处理dtw时间序列
    {
        ...
        float proScore = 0.0f;
        string scoreLevel;
        if(percentScore > 93.5f)
        {
            proScore = 90f;
            scoreLevel = "优";
        }else if(percentScore > 91.5 && percentScore < 93.5)
        {
            proScore = 85f;
            scoreLevel = "良";
        }else if(percentScore > 89 && percentScore < 91.5)
        {
            proScore = 80f;
            scoreLevel = "及格";
        }
        ...
        string allScoreStr = " 动作序列参考评分为: " + percentScore.ToString("F2") + "\n 对应的专家评分为: "  + proScore + "\n 对应的评价等级为：" + scoreLevel;
        outComeText.text = allScoreStr;
